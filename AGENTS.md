...
- Workspace lints (enforced via root `Cargo.toml`): `unsafe_code = "deny"`, `unused_must_use = "deny"`, `clippy::pedantic = "warn"`, `clippy::cargo = "warn"`, `clippy::disallowed_types = "deny"`, `clippy::non_std_lazy_statics = "deny"`. `rustfmt.toml` sets `max_width = 120`. If `unsafe` is unavoidable (e.g. FFI, shared memory), add `#[allow(unsafe_code)]` at the narrowest scope with a `// SAFETY:` comment explaining why it is sound. Before adding an external crate, check whether `std` already provides the functionality for the workspace's `rust-version`. Backport crates whose types are now in `std` (e.g. `once_cell`, `lazy_static`) are forbidden — see `clippy.toml` for the full list.
