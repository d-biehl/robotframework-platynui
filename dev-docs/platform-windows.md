...
## 2a. Java provider (`provider-java`) and its backends (agent, JAB)

There is exactly **one** registered Java provider: `crates/provider-java` (`platynui-provider-java`, `cfg(windows)`, descriptor id `java`, technology `Java`, `event_capabilities: None`), registered via `platynui_link_os_providers!` alongside the UIA provider. It is a router: per top-level window it delegates to a toolkit **backend**, and backend nodes travel to the runtime unwrapped, so `@Technology`, patterns and `UiNode::is_valid` stay the backend's own answers. A window is claimed exactly when a backend can serve it — a Java window no backend reaches (an SWT or JavaFX window without an agent) stays unclaimed and with the UIA provider rather than being claimed and served empty. Because there is only ever one Java claimant, window claims stay boolean; gaining a backend changes which backend *serves* a window, never who *claims* it. The router also owns the enablement diagnostic below, since only it knows whether some other backend reached the window. Full design: OpenSpec change `unify-java-provider`.

**Backend selection is the backend *order*, and nothing else.** The backends sit in preference order, strongest first — in-JVM agent, then Access Bridge — and the router records what each one served before the next runs, so a weaker backend already sees the windows it must leave alone *in the same pass*. The agent backend only reports windows of JVMs that carry an agent, so "prefer the agent when one is present, else JAB" needs no condition anywhere. Ownership is preference-aware rather than first-come: when an agent appears in a JVM the bridge has been serving, the agent takes the window over on the next pass. Selection is the backend's *input*, not a filter on its output, because an `app:Application` node enumerates its windows lazily — after the pass returned — so a post-filter would miss exactly those (`WindowExclusions` in the JAB crate, `BackendOwnership`/`ForeignWindows` in the router). The whole user-facing surface of this is `providers.java.*`: no attach keyword, no CLI subcommand.
...
- **The toolkit is a `native:` fact, not the technology.** `@Technology` names the *channel* (`JavaAgent` vs `JAB`) and has to keep doing so — it is what tells an agent-served window from a bridge-served one, which the dedup and routing suites assert on. Which Java toolkit a window belongs to is `native:JvmToolkit`, on the same names the bridge and the UIA shell publish, so the question has one answer regardless of who served the window. The agent's answer is the **authoritative** one that `JavaToolkit`'s own documentation anticipated: it maps the loaded-class detection from inside the process (`swing`/`awt`/`javafx`/`swt`) onto the shared `Swing/AWT` / `JavaFX` / `SWT` labels, where the platform classifier can only infer from a top-level window class. The raw set travels alongside as `native:JvmToolkits`, so a JVM running more than one toolkit — the deferred mixed case — is not flattened away. `IsJvm` and `JvmAgentPresent` are certainties here rather than probes; `JvmAccessibilityReachable` is deliberately absent, because it is about native accessibility, which the agent is not.
- **Identity-stable RuntimeIds** (`agent/<pid>/<element-id>`): the agent's ids are identity-based, so a relayout that reorders siblings renames nothing — the fundamental limit of JAB's enumeration-index scheme.
- **Virtual children have real identity**: the JDK hands out a *fresh* `AccessibleContext` wrapper per lookup for a JTable cell or JList entry, so the agent interns its own value object per `(owner, position)` instead of registering the wrapper. Measured on the fixture: the same cell keeps its id across enumerations, carries its own name from the model, its own rectangle, and its own selection state.
- **Off-view content has no place on screen, and says so.** A table is routinely larger than the viewport showing it, and `getCellRect` answers from the model regardless — measured on the fixture, row 90 of a 100-row scrolling table reports a rectangle ~2000 px below the window. The agent therefore intersects a cell's and a row's rectangle with the table's `getVisibleRect()`: nothing visible means **no `Bounds`, no `ActivationPoint`, and `IsInView = false`**, while partially scrolled content is *clipped* so the rectangle's centre — where pointer input aims — stays inside the part the user can see. The node keeps its name, coordinates and identity; only its claim to a screen position goes away. Same doctrine as `SwingGeometry.hasArea` for unlaid-out components: absent is the honest answer, a rectangle pointing at another application is not.
- **A table's shape follows the backend.** Through the agent a table is `Table` → `item:TableRow` → `item:TableCell`; through the bridge it stays one flat, row-major list of cells. The flat shape is not a model — it is what `AccessibleContext.getAccessibleChild(i)` offers, and the bridge has nothing else to offer — while the agent reads the model, where a row is a first-class thing with its own extent (`native:TableRow.Index`, `native:TableRow.IsSelected`, bounds spanning its cells) exactly as `provider-atspi` and `provider-windows-uia` already surface rows. The divergence is real and deliberate, and `@Technology` is how a consumer tells the two apart: a locator addressing cells positionally as direct children of a table works on one backend and not the other. One consequence worth knowing: `AccessibleSelection` on a `JTable` answers in *cell* indices, which no longer address a direct child, so the agent re-derives the table's selection from `getSelectedRows()` — `control:SelectedItems` names the selected **rows**, and cell-level selection stays on the cell. Full design: OpenSpec change `java-agent-table-rows`.
- **Native window handle**: read in-JVM from the AWT peer (`WComponentPeer#getHWnd`). Measured: on JDK 9+ that needs `java.awt` **and** `sun.awt.windows` opened, and the agent opens them to itself via `Instrumentation.redefineModule` — so no `--add-opens` on the command line, on any JDK. The provider's PID **plus geometry** match against `EnumWindows` is the genuine fallback; geometry is what distinguishes it from the platform layer's PID-only path, which by its own comment "can target the wrong sibling window", and an ambiguous match is refused rather than guessed.
- **Bounded, and honest when it is not**: consecutive timeouts mark a session degraded, after which calls fail fast until a rate-limited probe recovers it. A degraded or unreachable agent reports `is_valid() == false` rather than an optimistic `true`, so a scoped root re-resolves instead of pinning an element in a process that may be gone.
- **Recovery needs a new connection, not just a retry.** A call abandoned at its deadline was *abandoned*, not cancelled: the agent may still write that answer later, and the next call would read it as its own. The client notices the crossed ids and reports a protocol error, but it cannot un-cross them — so the session drops a connection that timed out and rebuilds it from the handshake file on the next call. Without this a single wedged moment made that JVM unusable for the rest of the run, which is what `live_frozen_agent_stays_contained` pins.
- **No text write and no highlight.** Text is typed via synthesized keyboard input (`text-input-policy`); `TextEditable` is a capability marker with no instance. Highlighting stays `HighlightProvider`'s (platform) job — what JAB lacks for a table cell is *bounds*, not a way to draw, so supplying bounds fixes the picker for every consumer at once and the agent never paints into the target.

### The Java Access Bridge backend (Swing/AWT, zero-config floor)

The second backend is the Java Access Bridge. Java Swing/AWT apps implement no UIA provider, so their windows are empty shells to the UIA provider. `crates/provider-java-jab` (`platynui-provider-java-jab`, `cfg(windows)`, technology `JAB`) reads them through the JDK's own out-of-process channel — the same one screen readers use. **Nothing is loaded into the target JVM** beyond the JDK's own bridge, and the backend never mutates target-side configuration (no `.accessibility.properties`, no registry writes; pinned by the `no_configuration_mutation_code_paths_exist` unit test). It is a library crate, not a registered provider, and is structured after `provider-atspi`. Full design: OpenSpec change `add-jab-provider`.
...
- Services all API calls from the typed `JabClient` (`client.rs`) over an mpsc channel, each with a per-call deadline (`providers.java.jab.call_timeout_ms`, default 2000 ms). No JAB function is ever called from another thread.
...
**Config keys**: `providers.java.enabled` (default true, the umbrella kill switch) and the backend sub-map `providers.java.jab.enabled` (default true), `providers.java.jab.dll_path`, `providers.java.jab.call_timeout_ms` (default 2000); `providers.java.agent.*` is reserved for the in-JVM agent backend. `providers.windows-uia.honor_window_claims` (default true) is unchanged. The pre-rename `providers.jab.*` spelling is gone — not aliased and not diagnosed; no released version ever read it.
