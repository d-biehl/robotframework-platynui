...
5. *In-process agent* — the route the commercial Java UI-test tools take (QF-Test, Squish for Java, Jubula load an agent into the target JVM via `-javaagent`/attach and bypass JAB entirely; full fidelity incl. `getCellRect`). Conflicts with the project's never-instrument principle (documented here and pinned by the `no_configuration_mutation_code_paths_exist` test) — adopting it would be a deliberate policy decision, not a technical tweak.
6. *Upstream OpenJDK fix* — make the bridge return the `AccessibleJTableCell` instead of the renderer (the workaround dates back to the JDK-6 era). Correct and durable, but years of lead time and never available for existing JDK-8 target apps. `AccessibleKeyBindings`/`AccessibleRelationSet` have no bit in the bitfield, so `KeyBindings.Bindings`/`RelationSet.Relations` are listed ungated and probe at value-read time (absent data reads as Null). Handle-bearing results (table caption/summary/`AccessibleTable`, relation targets, hyperlink handles) are released on the pump thread right after extraction; reads are live per access, and a degraded `vmID` yields no interface attributes instead of hanging.
