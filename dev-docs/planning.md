...
- **Rust toolchain**: MSRV `rustc 1.95.0`. Track release notes and adopt new features as appropriate.
...
- [x] Predicate pushdown — high impact, push filters closer to axis traversal (`crates/xpath/src/compiler/optimizer.rs`: `push_down_predicates()` merges `ApplyPredicates` into `AxisStep`)
- [x] `evaluate_first()` fast path — stop after first match (`crates/xpath/src/engine/evaluator/mod.rs`: streams and stops at first match, ~230x faster)
...
- [x] Comprehensive streaming predicate tests — 35 tests covering all positional operators, edge cases (zero, negative), sibling axes with predicates, nested/chained predicates, `last()`-based predicates (`crates/xpath/tests/it/evaluator_streaming.rs`)
