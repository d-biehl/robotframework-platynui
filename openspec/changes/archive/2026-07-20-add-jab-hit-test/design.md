...
1a. *Discovered during implementation — geometric fallback required.* The JDK implements `getAccessibleContextAt` for Swing/AWT apps via `EventQueueMonitor.getAccessibleAt`, which returns null for **every** point until the target JVM has observed at least one mouse event over one of its windows (`currentMousePosition == null`; screen readers never notice because they hit-test on mouse-move). A fresh JVM the pointer never touched therefore answers nothing even for its own window center. When the native hit-test yields no context, the provider descends geometrically instead: from the window context, pick the first *showing* child whose calibrated bounds contain the point (AWT child order approximates z-order — index 0 topmost, so overlays win), bounded by the same depth/child-scan limits as the reveal re-walk. A point over the window but outside every child (frame/title-bar area) resolves to the window itself.
