...
- **Test compositor:** `apps/wayland-compositor` already implements `xdg_toplevel` move/maximize/unmaximize grabs and honors client-side decoration requests, so the lane exercises the real protocol path end to end. Implementing the acceptance suite exposed one real bug there (fixed in this change): the pointer and touch move grabs truncated each motion event's delta to whole pixels, and since per-event deltas from high-frequency sources (virtual pointers, high-rate mice) are routinely sub-pixel, an entire drag could truncate to zero movement — the grabs now accumulate the fractional remainder.
