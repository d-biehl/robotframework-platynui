...
- **Depends on**: `java-agent-core` (agent artifact, transport, attach, delivery), `provider-java-swing` (the client, mapping layer and backend routing), **`java-provider-linux`** (for the Linux half — the Java provider does not run there until then) and `add-swt-test-app` (the fixture + catalog suite).
- **Open for the Linux half**: SWT is GTK-native, so unlike Swing it *does* register on the accessibility bus and is visible to `provider-atspi`. An agent-served SWT window on Linux therefore has two representations, and `provider-atspi` consults no window claims — the single-appearance guarantee is Windows-only today (only `provider-windows-uia` implements the abstain). Settle this here: either AT-SPI learns to honor claims, or the session restricts providers (`runtime-provider-selection`).
