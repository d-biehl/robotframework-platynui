...
- **Same self-bootstrapping toolchain as the SWT fixture** (`add-swt-test-app` establishes the pattern): checked-in Gradle wrapper running on the PATH JDK (8+, Gradle 8.x pinned), Java 21 toolchain auto-provisioned via the Foojay resolver — nothing to install manually. JavaFX resolves from Maven Central via the official `org.openjfx.javafxplugin` Gradle plugin (handles platform classifiers, natives, and the module path). Unlike the SWT fixture there is **no Java 8 target/runtime lane**: OpenJFX exists on Maven Central only from version 11, and JavaFX-on-8 lives solely in FX-bundled JDK 8 distributions (Zulu FX / Liberica Full) — documented as out of scope in the design.
...
- **Depends on**: nothing hard — shares the toolchain pattern with `add-swt-test-app`, and implementing that first avoids re-deciding it. **Unblocks**: real-app JavaFX coverage for `java-app-classification`; the primary target for `provider-java-agent`.
