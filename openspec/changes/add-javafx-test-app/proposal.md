...
- **Catalog-suite onboarding**: a thin `tests/acceptance/javafx/catalog.robot` supplies only the JavaFX launch configuration and runs the shared catalog keywords (reference implementation lands with `add-qml-test-app`); JavaFX/UIA limitations surface as documented skips per the blueprint.
