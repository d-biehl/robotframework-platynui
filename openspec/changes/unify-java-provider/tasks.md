...
- [ ] 1.1 New crate `crates/provider-java` (name `platynui-provider-java`): UiTree-provider surface, per-window router, backend trait shaped after the existing per-window provider surface (discover/claim, serve subtree, patterns, degraded tracking)
...
- [ ] 1.4 Claim-owner id becomes the Java provider (`"java"` instead of `"jab"`) in `window_claims` and the UIA abstain check — a consequence of the single-claimant decision, not of the config rename; update the affected tests
