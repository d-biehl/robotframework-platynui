...
- **Depends on**: `java-agent-core` (agent artifact, transport, attach, delivery), `provider-java-swing` (the client, mapping layer and backend routing; transitively `unify-java-provider`), **`java-provider-linux`** (the Java provider does not run on Linux at all until then — and Linux is this change's whole point, so its Linux lane is blocked without it) and `add-javafx-test-app` (the fixture + catalog suite).
