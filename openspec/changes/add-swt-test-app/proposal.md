...
- **Catalog-suite onboarding**: `tests/acceptance/swt/catalog.robot` replicates the blueprint's canonical catalog test set as a self-contained suite (reference implementation lands with `add-qml-test-app`) with the SWT launch configuration; SWT-specific limitations surface as documented skips per the blueprint.
...
- **Depends on**: `test-app-blueprint` (catalog, canonical names, CLI, suite conventions) and `add-qml-test-app` (the canonical catalog test set this fixture's `catalog.robot` replicates). The Gradle scaffold, runtime selection (`writeJavaLaunchers` → `build/java-launchers.properties`, per-app `PLATYNUI_TEST_APP_*` env vars), and daemon JVM criteria are established by the archived `migrate-swing-test-app-to-gradle` change and reused here. **Unblocks**: real-app SWT coverage for `java-app-classification`; an SWT target for `provider-java-swt`; the SWT row of the fixture technology matrix.
