...
- [The fixture's table is small (4×3), so the shape is proven but not stressed] → true, and deliberate: correctness first. The large-table behaviour is a separate, already-recorded concern.
