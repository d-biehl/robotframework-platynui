...
- **Per-instance scoped state.** `Set Root` and `Set Query Settings` store their value under a variable name derived from the name the calling instance is registered under in the Robot Framework namespace: the default import keeps `${PLATYNUI_ROOT_DESCRIPTOR}` / `${PLATYNUI_QUERY_SETTINGS}`, an aliased import gets a normalized `_<ALIAS>` suffix. The name is stable across the per-suite instances that `scope='SUITE'` creates (`:238-244`), so suite-scoped roots keep being inherited by child suites.
- **A suite-scoped value reaches the suites below it.** `Set Root`/`Set Query Settings` with `scope=SUITE` currently call `variables.set_suite(...)` with Robot Framework's default `children=False`, so the value stops at the suite that set it — a directory's `__init__.robot` cannot pin the context for the suites it contains, which is why every acceptance suite repeats the launch-and-pin flow itself. They now propagate to child suites.
- **Origin fingerprint as a safety net.** Stored values carry a fingerprint of their originating instance's import arguments. A value inherited by an instance with a different fingerprint (same registered name, different import args across a suite boundary — reachable precisely because of the propagation above) SHALL be ignored with a warning instead of resolved through the wrong session.
