...
- **Shared catalog suite (reference implementation)**: technology-neutral catalog keywords in `tests/acceptance/resources/` per the blueprint conventions, plus the first thin onboarding suite `tests/acceptance/qml/catalog.robot` and QML-specific suites for the popup-mode and custom-control coverage.
...
- `qml-test-app`: the Qt Quick/QML fixture — blueprint-conforming core catalog with QML `Accessible` wiring, dual popup modes, implemented custom-controls chapter, and the first onboarding of the shared catalog acceptance suite on Windows (UIA) and Linux (AT-SPI).
...
- **New**: `apps/test-app-qml/**` (`main.py`, QML sources, README), `tests/acceptance/resources/` catalog resource, `tests/acceptance/qml/**` suites.
