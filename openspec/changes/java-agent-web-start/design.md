...
- *`SunToolkit.invokeLaterOnAppContext`.* Does exactly the right thing, but is a deeper internal than the `AppContext` accessors and has moved between JDK versions; the two accessors used here (`getAppContexts`, `get`) have been stable since 1.4 and are what the AWT code itself uses.
...
- **[The structural-change hint may not cross `AppContext`s]** → `SwingAdapter.watchStructuralChanges` registers a global `AWTEventListener`. Whether it observes events dispatched on another context's event queue is not established, and it is now reachable at all only because bootstrap loading grants `listenToAllAWTEvents` (the existing `SecurityException` fallback stops triggering). If it does not cross, a Web Start application's tree changes without the generation counter moving, so clients keep a stale view until they poll. This degrades a hint, not correctness — per-element validity has its own endpoint and is unaffected — but it is the difference between an Inspector that refreshes itself and one that does not. To be measured against the second-context fixture, and registered per context if the measurement says so.
