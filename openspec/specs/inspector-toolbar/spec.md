# inspector-toolbar Specification

## Purpose

The Inspector's header-area contract. A themed toolbar directly below the menu bar hosts the primary actions — the live-picker toggle, the node actions (refresh, refresh subtree, highlight), and the always-on-top window toggle — while the search row below it carries only the XPath field and its Search/Stop button. The toolbar has a configurable display style (icons only by default, icons with text), theme-following vector icons, tooltips with shortcuts, and stable element IDs plus mode-independent accessible names so acceptance suites and assistive technology address the controls reliably.
