# compositor-popup-geometry Specification

## Purpose

A Wayland client cannot know its own global position, so toolkit-reported screen extents for transient popups (context menus, dropdowns) are client-local and useless for pointer interaction. The PlatynUI compositor places every popup itself and therefore knows their real screen rectangles; this capability exposes that geometry over the control socket and routes it through the platform window manager into the AT-SPI provider, so popup interaction under the compositor is exact.
