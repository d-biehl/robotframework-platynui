# uia-common-attributes Specification

## Purpose
The common-attribute surface of the Windows UIAutomation provider: the always-present
`Technology` and `SupportedPatterns` attributes, the conditions under which `Focusable`
and the `TextEditable` capability marker are advertised, and the `IsReadOnly` attribute
on text-bearing elements. Also covers the shared common-attribute expectation in the
core contract testkit that provider suites reuse.
