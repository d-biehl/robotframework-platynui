# inspector-live-mouse-picker Specification

## Purpose

The fastest way a user identifies a control is by pointing at it. This capability is the Inspector's live picking mode (the Accessibility-Insights-style workflow): behind an explicit arming toggle, while the user holds a configurable modifier combination (default Ctrl+Alt+Shift), the Inspector continuously resolves the element under the mouse cursor via the `element-at-point` capability and drives its existing reveal, select, and highlight machinery — following the cursor while held, stopping on release, and cleanly disabled (greyed out, with the reason discoverable) where live cursor position or hit-test is unavailable.
