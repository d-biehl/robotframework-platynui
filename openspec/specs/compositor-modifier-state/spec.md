# compositor-modifier-state Specification

## Purpose

The Inspector's live picker needs to observe globally held modifier keys (Ctrl+Alt+Shift) while the Inspector itself is unfocused. Under the PlatynUI compositor no X server can answer that — and a leaked host `DISPLAY` must not be consulted, because the host seat never sees input injected into the compositor. This capability exposes the compositor seat's keyboard modifier state over the control socket and makes the Inspector prefer it, so live picking works inside compositor sessions.
