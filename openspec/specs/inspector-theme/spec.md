# inspector-theme Specification

## Purpose

The Inspector's theme contract: a persisted System/Light/Dark preference (System is the default), where System follows the operating system's color scheme — the windowing system's report on Windows/macOS, the XDG Desktop Portal's `color-scheme` on Linux, with live updates and a deterministic dark fallback when no signal is available — plus an ephemeral command-line/environment override that pins the theme for a single run (used by scripted test passes) without ever touching the settings file.
