...
def test_pointer_overrides_accept_origin_desktop_point_rect():
    ...
    rt = Runtime()
    _assert_frompyobject_ok(lambda: rt.pointer_move_to(Point(0.0, 0.0), overrides=PointerOverrides(origin="desktop")))
    _assert_frompyobject_ok(lambda: rt.pointer_move_to(Point(0.0, 0.0), overrides=PointerOverrides(origin=Point(1.0, 2.0))))
    _assert_frompyobject_ok(lambda: rt.pointer_move_to(Point(0.0, 0.0), overrides=PointerOverrides(origin=Rect(1.0, 2.0, 3.0, 4.0))))
...
def test_pointer_button_enum_is_accepted():
    ...
    rt = Runtime()
    _assert_frompyobject_ok(lambda: rt.pointer_click(Point(0.0, 0.0), button=PointerButton.LEFT))
    _assert_frompyobject_ok(lambda: rt.pointer_click(Point(0.0, 0.0), button=PointerButton.MIDDLE))
    _assert_frompyobject_ok(lambda: rt.pointer_click(Point(0.0, 0.0), button=PointerButton.RIGHT))
