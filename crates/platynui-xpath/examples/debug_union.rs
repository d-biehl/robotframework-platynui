...
fn sample() -> Node { let mut d=Dom{nodes:vec![]}; let root=el(&mut d,None,"root"); let a1=el(&mut d,Some(root),"a"); at(&mut d,a1,"id","x"); let _b=el(&mut d,Some(a1),"b"); let a2=el(&mut d,Some(root),"a"); at(&mut d,a2,"id","y"); let c=el(&mut d,Some(a2),"c"); tx(&mut d,c,"hello"); Node{dom:Arc::new(d), idx:root} }
    ...
    for expr in ["a", "//c", "a | //c", "a/descendant::a", "a/descendant::c"] {
        if false {
            ...
            fn walk(pair: pest::iterators::Pair<Rule>, depth: usize) {
                println!("{:indent$}{:?} -> '{}'", "", pair.as_rule(), pair.as_str(), indent=depth*2);
                for p in pair.clone().into_inner() { walk(p, depth+1); }
            }
            for p in pairs { walk(p, 0); }
        }
