use platynui_xpath::{evaluate_expr, ExpandedName, XdmItem as I, xdm::XdmAtomicValue as A, SimpleNode};
use platynui_xpath::runtime::{DynamicContextBuilder, FunctionRegistry, CallCtx, Error};
use rstest::rstest;
type N = SimpleNode;
...
fn variables_and_functions() {
    ...
    reg.register(ExpandedName{ ns_uri: ns.clone(), local: "twice".to_string() }, 1, 
        std::sync::Arc::new(|_ctx: &CallCtx<N>, args: &[Vec<I<N>>]| -> Result<Vec<I<N>>, Error> {
            let v = match &args[0][0] { I::Atomic(A::Integer(i)) => *i, _ => 0 };
