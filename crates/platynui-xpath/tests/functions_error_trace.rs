...
fn error_with_custom_code() {
    let c = ctx();
    let err = evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("error('err:XPST0003')", &c).unwrap_err();
...
fn error_with_desc_and_data() {
    let c = ctx();
    let err =
        evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("error('err:FORG0001', 'bad cast', 123)", &c).unwrap_err();
