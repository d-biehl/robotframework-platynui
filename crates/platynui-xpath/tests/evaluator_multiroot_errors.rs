...
fn multiroot_union_errors() {
    ...
    let ctx = platynui_xpath::runtime::DynamicContextBuilder::<Node>::new()
        .with_variable(ExpandedName { ns_uri: None, local: "n1".into() }, vec![XdmItem::Node(n1)])
        .with_variable(ExpandedName { ns_uri: None, local: "n2".into() }, vec![XdmItem::Node(n2)])
        .build();
