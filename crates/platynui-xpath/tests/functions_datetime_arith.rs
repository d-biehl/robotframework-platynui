...
fn datetime_add_daytimeduration() {
    ...
    let str_dt = compile_xpath(
        "string('2024-01-02T03:04:05+00:00' cast as xs:dateTime)",
        &sc,
    )
    .unwrap();
    ...
    let expr =
        "('2024-01-02T03:04:05+00:00' cast as xs:dateTime) + ('P1DT2H' cast as xs:dayTimeDuration)";
...
fn date_add_yearmonthduration() {
    ...
    let exec = compile_xpath(
        "string(('2024-01-31+02:00' cast as xs:date) + ('P1M' cast as xs:yearMonthDuration))",
        &sc,
    )
    .unwrap();
...
fn duration_mul_div() {
    ...
    let exec2 = compile_xpath(
        "('P2Y' cast as xs:yearMonthDuration) div ('P6M' cast as xs:yearMonthDuration)",
        &sc,
    )
    .unwrap();
    let out2: Vec<XdmItem<DummyNode>> = exec2.evaluate(&Default::default()).unwrap();
    assert!(
        matches!(&out2[0], XdmItem::Atomic(XdmAtomicValue::Double(d)) if (*d - 4.0).abs() < 1e-9)
    );
    ...
    // ymd + ymd
    let exec4 = compile_xpath(
        "string(('P2Y' cast as xs:yearMonthDuration) + ('P6M' cast as xs:yearMonthDuration))",
        &sc,
    )
    .unwrap();
