...
fn exactly_one_happy() {
    let c = ctx();
    let out = evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("exactly-one((42))", &c)
        .unwrap();
    assert_eq!(out.len(), 1);
}
...
fn one_or_more_happy() {
    let c = ctx();
    let out = evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("one-or-more((1,2))", &c)
        .unwrap();
    assert_eq!(out.len(), 2);
}
...
fn zero_or_one_happy() {
    let c = ctx();
    let out =
        evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("zero-or-one((1))", &c).unwrap();
    assert_eq!(out.len(), 1);
}
