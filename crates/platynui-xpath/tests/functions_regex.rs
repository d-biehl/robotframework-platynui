...
fn matches_dotall_flag() {
    let sc = StaticContext::default();
    let exec_true = compile_xpath(
        r#"matches("a
b", 'a.b', 's')"#,
        &sc,
    )
    .unwrap();
    let exec_false = compile_xpath(
        r#"matches("a
b", 'a.b')"#,
        &sc,
    )
...
#[rstest]
fn replace_invalid_flag_errors() {
    let sc = StaticContext::default();
    let exec = compile_xpath("replace('a','a','b','z')", &sc).unwrap();
    let err = exec
        .evaluate(&platynui_xpath::runtime::DynamicContextBuilder::<DummyNode>::new().build())
        .expect_err("FORX0002 expected");
    assert_eq!(err.code, "err:FORX0002");
...
#[rstest]
fn tokenize_invalid_flag_errors() {
    let sc = StaticContext::default();
    let exec = compile_xpath("tokenize('a','a','z')", &sc).unwrap();
    let err = exec
        .evaluate(&platynui_xpath::runtime::DynamicContextBuilder::<DummyNode>::new().build())
        .expect_err("FORX0002 expected");
    assert_eq!(err.code, "err:FORX0002");
