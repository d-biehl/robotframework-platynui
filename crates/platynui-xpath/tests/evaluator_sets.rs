...
fn names<T: XdmNode>(items: &Vec<platynui_xpath::xdm::XdmItem<T>>) -> Vec<String> {
    use platynui_xpath::xdm::XdmItem;
    let mut v = vec![];
    for it in items {
...
#[rstest]
fn test_union_intersect_except(root: Node, sc: StaticContext) {
    // union as OP_PIPE: a | //c -> [a, a, c]
    let exec = compile_xpath("a | //c", &sc).unwrap();
    ...
    let exec = compile_xpath("a intersect //c", &sc).unwrap();
    let out: Vec<XdmItem<Node>> = exec.evaluate_on(Some(root.clone())).unwrap();
    assert!(names(&out).is_empty());

    // except: remove all a via self-except
    let exec = compile_xpath("a except a", &sc).unwrap();
    let out: Vec<XdmItem<Node>> = exec.evaluate_on(Some(root.clone())).unwrap();
    assert!(names(&out).is_empty());
}

#[rstest]
fn test_sibling_axes(root: Node, sc: StaticContext) {
    // simple sibling: first a has a following-sibling a
    let exec = compile_xpath("a/following-sibling::a", &sc).unwrap();
