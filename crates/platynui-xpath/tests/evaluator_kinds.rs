...
fn el(dom: &mut Dom, p: usize, local: &str) -> usize {
    let i = dom.nodes.len();
    dom.nodes.push(NodeRec {
...
fn cm(dom: &mut Dom, p: usize, v: &str) -> usize {
    let i = dom.nodes.len();
    dom.nodes.push(NodeRec {
...
fn pi(dom: &mut Dom, p: usize, target: &str, v: &str) -> usize {
    let i = dom.nodes.len();
    dom.nodes.push(NodeRec {
...
fn sample_document() -> Node {
    ...
    Node {
        dom: Arc::new(d),
        idx: r,
    } // context item as the document element
}

fn kind_names(items: &Vec<XdmItem<Node>>) -> Vec<NodeKind> {
    items
        .iter()
        .filter_map(|it| {
