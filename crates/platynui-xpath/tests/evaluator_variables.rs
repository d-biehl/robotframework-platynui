...
use platynui_xpath::runtime::{DynamicContextBuilder, StaticContext};
...
fn sample() -> Node {
    ...
    let a1 = el(&mut d, Some(r), "a");
    at(&mut d, a1, "id", "x");
    let _a2 = el(&mut d, Some(r), "a");
    Node {
        dom: Arc::new(d),
        idx: r,
    }
...
fn variable_arithmetic(root: Node, sc: StaticContext) {
    ...
    let ctx = DynamicContextBuilder::new()
        ...
        .with_variable(
            ExpandedName::new(None, "x"),
            vec![XdmItem::Atomic(XdmAtomicValue::Integer(3))],
        )
        .build();
...
fn variable_in_predicate(root: Node, sc: StaticContext) {
    ...
    let ctx = DynamicContextBuilder::new()
        .with_context_item(XdmItem::Node(root))
        .with_variable(
            ExpandedName::new(None, "id"),
            vec![XdmItem::Atomic(XdmAtomicValue::String("x".into()))],
        )
        .build();
    let out: Vec<XdmItem<Node>> = exec.evaluate(&ctx).unwrap();
    assert_eq!(out.len(), 1);
