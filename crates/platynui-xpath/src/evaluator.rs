...
impl XPathExecutable {
    pub fn evaluate<N: crate::model::XdmNode>(&self, _dyn_ctx: &DynamicContext<N>) -> Result<XdmSequence<N>, Error> {
    ...
    fn run_instrs<N: crate::model::XdmNode>(&self, instrs: &crate::compiler::InstrSeq, dyn_ctx: &DynamicContext<N>, position: i64, last: i64) -> Result<XdmSequence<N>, Error> {
        ...
        while ip < code.len() {
            match &code[ip] {
                ...
                OpCode::Pop => {
                    stack.pop().ok_or_else(|| Error::dynamic_err("err:FOER0000", "stack underflow"))?;
                }
                OpCode::JumpIfTrue(off) => {
                    let top = stack.last().ok_or_else(|| Error::dynamic_err("err:FOER0000", "stack underflow"))?;
                    let b = ebv(top)?;
                    if b { ip += off; continue; }
                }
                ...
                OpCode::ToRoot => {
                    let seq = stack.pop().ok_or_else(|| Error::dynamic_err("err:FOER0000", "stack underflow"))?;
                    let mut out = Vec::new();
                    let nodes = node_seq(seq)?;
                    for n in nodes { out.push(XdmItem::Node(root_of(n))); }
                    stack.push(out);
                }
                OpCode::AxisStep(axis, test, preds) => {
                    ...
                    for n in nodes.drain(..) {
                        ...
                        for s in step_nodes { if matches_test(&s, test) { acc.push(s); } }
                    }
                    // dedup
                    let mut unique: Vec<N> = Vec::new();
                    'outer: for n in acc.into_iter() {
                        for u in &unique { if *u == n { continue 'outer; } }
                        unique.push(n);
                    }
                    ...
                    for pred in preds {
                        ...
                        for (idx, node) in filtered.into_iter().enumerate() {
                            ...
                            if predicate_truthy(&res, pos)? { next.push(node); }
                        }
                        filtered = next;
                    }
                ...
                OpCode::MakeSeq(n) => {
                    ...
                    for _ in 0..*n {
                        let mut s = stack.pop().ok_or_else(|| Error::dynamic_err("err:FOER0000", "stack underflow"))?;
                ...
                OpCode::CallByName(name, argc) => {
                    ...
                    for _ in 0..*argc { args.push(stack.pop().ok_or_else(|| Error::dynamic_err("err:FOER0000", "stack underflow"))?); }
                    args.reverse();
                    if let Some(fun) = dyn_ctx.functions.get(name, *argc) {
                        let res = (fun)(&args)?;
                        stack.push(res);
                    } else {
                        return Err(Error::static_err("err:XPST0017", format!("unknown function {} with arity {}", name.local, argc)));
                    }
                }
                ...
                OpCode::RangeTo => {
                    ...
                    if la.len() != 1 || ra.len() != 1 {
                        return Err(Error::dynamic_err("err:FORG0006", "range expects exactly one atomic value per side"));
                    }
                    let start = as_integer(&la[0])?;
                    let end = as_integer(&ra[0])?;
                    let mut out: XdmSequence<N> = Vec::new();
                    if start <= end {
                        for i in start..=end { out.push(XdmItem::Atomic(XdmAtomicValue::Integer(i))); }
                    }
                    stack.push(out);
                }
                OpCode::NodeIs => {
...
fn take2<N>(stack: &mut Vec<XdmSequence<N>>) -> Result<(XdmSequence<N>, XdmSequence<N>), Error> {
    let r = stack.pop().ok_or_else(|| Error::dynamic_err("err:FOER0000", "empty stack"))?;
    let l = stack.pop().ok_or_else(|| Error::dynamic_err("err:FOER0000", "empty stack"))?;
    Ok((l, r))
}
...
fn bin_bool<N, F: FnOnce(bool, bool) -> bool>(op: F, stack: &mut Vec<XdmSequence<N>>) -> Result<(), Error> {
    let (l, r) = take2(stack)?;
    let lb = ebv(&l)?;
    let rb = ebv(&r)?;
...
fn the_number<N>(seq: &XdmSequence<N>) -> Result<f64, Error> {
    if seq.len() != 1 { return Err(Error::dynamic_err("err:FORG0006", "arithmetic expects one item")); }
    match &seq[0] {
        ...
        XdmItem::Atomic(XdmAtomicValue::UntypedAtomic(s)) => s.parse::<f64>().map_err(|_| Error::dynamic_err("err:FORG0001", "cannot cast untypedAtomic to number")),
        _ => Err(Error::dynamic_err("err:XPTY0004", "non-numeric in arithmetic")),
    }
}
...
fn bin_num2<N, F: FnOnce(f64, f64) -> Result<f64, Error>>(f: F, stack: &mut Vec<XdmSequence<N>>) -> Result<(), Error> {
    let (l, r) = take2(stack)?;
    let a = the_number(&l)?;
    let b = the_number(&r)?;
    let v = f(a, b)?;
...
fn num_add(a: f64, b: f64) -> f64 { a + b }
fn num_sub(a: f64, b: f64) -> f64 { a - b }
fn num_mul(a: f64, b: f64) -> f64 { a * b }
fn num_div(a: f64, b: f64) -> Result<f64, Error> { if b == 0.0 { Err(Error::dynamic_err("err:FOAR0001", "divide by zero")) } else { Ok(a / b) } }
...
fn atomize_sequence<N: crate::model::XdmNode>(s: &XdmSequence<N>) -> Result<Vec<XdmAtomicValue>, Error> {
    let mut out = Vec::new();
    for it in s { match it { XdmItem::Atomic(a) => out.push(a.clone()), XdmItem::Node(n) => out.push(XdmAtomicValue::UntypedAtomic(n.string_value())) } }
    Ok(out)
}
...
fn is_numeric(a: &XdmAtomicValue) -> bool {
    matches!(a, XdmAtomicValue::Integer(_) | XdmAtomicValue::Double(_) | XdmAtomicValue::Float(_) | XdmAtomicValue::Decimal(_))
}
...
fn as_integer(a: &XdmAtomicValue) -> Result<i64, Error> {
    match a {
        ...
        XdmAtomicValue::UntypedAtomic(s) => s.parse::<f64>().map_err(|_| Error::dynamic_err("err:FORG0001", "cannot cast untypedAtomic to number")).map(|v| v.floor() as i64),
        _ => Err(Error::dynamic_err("err:XPTY0004", "not a numeric value")),
    }
}
...
fn single_node<N>(seq: XdmSequence<N>) -> Result<N, Error> {
    let v = node_seq(seq)?;
    if v.len() != 1 { return Err(Error::dynamic_err("err:XPTY0004", "node comparison expects single node on each side")); }
    Ok(v.into_iter().next().unwrap())
}

fn root_of<N: crate::model::XdmNode>(mut n: N) -> N {
    while let Some(p) = n.parent() { n = p; }
    n
}
...
fn apply_axis<N: crate::model::XdmNode>(n: &N, axis: &AxisIR) -> Vec<N> {
    match axis {
        ...
        AxisIR::Ancestor => {
            ...
            while let Some(p) = cur { v.push(p.clone()); cur = p.parent(); }
            v
        }
        ...
        AxisIR::PrecedingSibling => {
            if let Some(parent) = n.parent() {
                ...
                for c in parent.children() {
                    if c.compare_document_order(n) == core::cmp::Ordering::Less { res.push(c); }
                }
                return res;
            }
            Vec::new()
        ...
        AxisIR::FollowingSibling => {
            if let Some(parent) = n.parent() {
                let mut res = Vec::new();
                for c in parent.children() {
                    if c.compare_document_order(n) == core::cmp::Ordering::Greater { res.push(c); }
                }
                return res;
            }
            Vec::new()
        ...
        AxisIR::Namespace => {
            ...
            if n.kind() == crate::model::NodeKind::Element { n.namespaces() } else { Vec::new() }
        }
    }
}
...
fn is_ancestor_of<N: crate::model::XdmNode>(a: &N, n: &N) -> bool {
    let mut cur = n.parent();
    while let Some(p) = cur { if &p == a { return true; } cur = p.parent(); }
    false
}
