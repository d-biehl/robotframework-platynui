...
| Path expressions (§3.2) | Root/relative paths, all forward/backward axes, namespace axis, filter steps, predicates | AST variants and lowering cover every axis (`crates/platynui-xpath/src/parser/ast.rs:191-228`, `crates/platynui-xpath/src/compiler/mod.rs:374-419`). Predicates maintain focus using VM frames (`crates/platynui-xpath/src/engine/evaluator.rs:140-189`). A step may start from a primary expression anywhere in the path, including directly after a leading `/` (`/.`, `/(a\|b)`). |
| Leading `/` abbreviation (§3.2) | ✅ incl. `treat as document-node()` | `OpCode::ToRoot` walks to the root and raises `XPDY0050` when it is not a document node, as required by `(fn:root(self::node()) treat as document-node())` (`crates/xpath/src/engine/evaluator/mod.rs`, `OpCode::ToRoot`). `fn:root()` itself is unaffected. |
| Path step type rules (§3.2) | ❌ missing | `XPTY0019` (non-node result in a non-final step) and `XPTY0018` (final step mixing nodes and non-nodes) are not checked; such paths silently yield an empty or mixed sequence. |
