...
// `t` is already truncated and only its parity is used; saturation is harmless.
...
fn round_half_to_even_bankers_matrix(
    #[values(-100_000.0, -2.5, -2.0, -1.5, -1.0, -0.5, -0.0, 0.0, 0.5, 1.5, 2.0, 2.5, 100_000.0)] x: f64,
    ...
    if let Some(r) = eval_double(&expr) {
        ...
        assert!((r - expected).abs() <= 1e-9, "x={x}, prec={prec}, r={r}, expected={expected}");
    } else {
