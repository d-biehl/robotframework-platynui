...
fn d(expr: &str) -> f64 {
    ...
    match seq.first() {
        ...
        Some(XdmItem::Atomic(XdmAtomicValue::Float(v))) => f64::from(*v),
        Some(XdmItem::Atomic(XdmAtomicValue::Decimal(v))) => rust_decimal::prelude::ToPrimitive::to_f64(v).unwrap(),
        // Test results are small integers, which f64 represents exactly.
...
// Exact equality intended: sums, minima and maxima of small integers are exact in f64.
...
fn sum_basic_and_empty(#[case] expr: &str, #[case] expected: f64) {
    assert_exact_f64(d(expr), expected);
}
    ...
    match expected {
        Some(val) => {
            let got = match seq.first().expect("value") {
                ...
                XdmItem::Atomic(XdmAtomicValue::Float(v)) => f64::from(*v),
