...
// Integral doubles/floats are printed as integers; test values fit in i64.
    ...
    result
        ...
        .map(|item| match item {
            ...
            XdmItem::Atomic(XdmAtomicValue::Decimal(d)) => {
                if d.is_zero() || d.fract().is_zero() {
                    format!("{}", rust_decimal::prelude::ToPrimitive::to_i64(d).unwrap_or(0))
                } else {
                    d.to_string()
                }
            }
