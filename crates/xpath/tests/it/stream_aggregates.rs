...
fn sum_mixed_numeric_types() {
    ...
    match &result[0] {
        XdmItem::Atomic(XdmAtomicValue::Decimal(d)) => {
            assert_eq!(*d, rust_decimal::Decimal::from_str_exact("6.5").unwrap());
        }
        _ => panic!("Expected decimal"),
    }
}
...
fn avg_decimals() {
    ...
    match &result[0] {
        XdmItem::Atomic(XdmAtomicValue::Decimal(d)) => {
            ...
            assert!(
                (d - expected).abs() < rust_decimal::Decimal::from_str_exact("0.01").unwrap(),
                "Expected ~3.333, got {d}"
            );
        }
        _ => panic!("Expected decimal"),
    }
...
fn combined_sum_avg() {
    ...
    match &result[0] {
        XdmItem::Atomic(XdmAtomicValue::Decimal(d)) => {
            assert_eq!(*d, rust_decimal::Decimal::from_str_exact("5.5").unwrap());
        }
