...
// Exact equality intended: number('2') is exactly 2.0.
...
fn subsequence_converts_numeric_arguments() {
    ...
    assert_eq!(result.len(), 1);
    match &result[0] {
        I::Atomic(A::String(s) | A::UntypedAtomic(s)) => assert_eq!(s, "Data"),
        I::Node(n) => assert_eq!(n.string_value(), "Data"),
        other @ I::Atomic(_) => panic!("unexpected result: {other:?}"),
    }
}
...
fn distinct_values_handles_untyped_atomic() {
    ...
    let mut values: Vec<String> = result
        .iter()
        .map(|i| match i {
            I::Atomic(A::String(s) | A::UntypedAtomic(s)) => s.clone(),
            other => panic!("unexpected result: {other:?}"),
        })
        .collect();
...
fn sum_accepts_duration_sequences(#[case] expr: &str, #[case] expected: A) {
    ...
    match &result[0] {
        ...
        other @ I::Node(_) => panic!("unexpected result: {other:?}"),
    }
}
...
// Exact equality intended: 1 + 2 is exactly 3.0.
#[allow(clippy::float_cmp)]
#[rstest]
...
// Exact equality intended: the rounded result must match the literal.
#[allow(clippy::float_cmp)]
#[rstest]
