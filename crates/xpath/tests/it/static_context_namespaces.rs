...
fn register_and_use_custom_prefix_in_qname_constructor() {
    ...
    assert_eq!(seq.len(), 1);
    let XdmItem::Atomic(atom) = &seq[0] else { panic!("expected atomic") };
...
fn resolve_qname_uses_static_namespace() {
    ...
    let seq = evaluate(&compiled, &ctx).unwrap();
    let XdmItem::Atomic(platynui_xpath::xdm::XdmAtomicValue::AnyUri(uri)) = &seq[0] else { panic!("expected anyURI") };
...
fn default_element_namespace_applies_to_unprefixed_steps() {
    ...
    let node = match &seq[0] {
        ...
        XdmItem::Atomic(_) => panic!("expected node"),
    };
