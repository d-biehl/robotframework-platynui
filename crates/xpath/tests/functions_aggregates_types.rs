...
fn sum_integers_stays_integer() {
    let c = ctx();
    let r =
        evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("fn:sum((1,2,3))", &c).unwrap();
    match &r[0] {
...
fn sum_seed_used_when_empty() {
    let c = ctx();
    let r =
        evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("fn:sum((), 42)", &c).unwrap();
    match &r[0] {
...
fn avg_integer_sequence_decimal_result() {
    let c = ctx();
    let r = evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("fn:avg((1,2,3,4))", &c)
        .unwrap();
    assert!(matches!(r[0], I::Atomic(A::Decimal(_))));
}
...
fn min_numeric_preserves_kind_integer() {
    let c = ctx();
    let r =
        evaluate_expr::<platynui_xpath::model::simple::SimpleNode>("fn:min((3,1,2))", &c).unwrap();
    match &r[0] {
