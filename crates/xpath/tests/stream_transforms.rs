...
fn combined_insert_remove() {
    let ctx = DynamicContextBuilder::default().build();
    let result = evaluate_expr::<SimpleNode>(
        "remove(insert-before((1, 3), 2, (2)), 4)",
        &ctx
    ).unwrap();
...
fn combined_remove_insert() {
    let ctx = DynamicContextBuilder::default().build();
    let result = evaluate_expr::<SimpleNode>(
        "insert-before(remove((1, 2, 3, 4), 2), 2, (2))",
        &ctx
    ).unwrap();
...
fn combined_count_insert_before() {
    let ctx = DynamicContextBuilder::default().build();
    let result = evaluate_expr::<SimpleNode>(
        "count(insert-before(1 to 100, 50, (999)))",
        &ctx
    ).unwrap();
