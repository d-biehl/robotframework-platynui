...
impl fmt::Debug for SimpleNode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let attr_count = self.0.attributes.read().map_or(0, |v| v.len());
        let ns_count = self.0.namespaces.read().map_or(0, |v| v.len());
        let child_count = self.0.children.read().map_or(0, |v| v.len());
        let cached = self.0.cached_text.read().is_ok_and(|c| c.is_some());
...
impl SimpleNode {
    ...
    pub fn document() -> SimpleNodeBuilder {
        ...
        *b.node.0.doc_id.write().unwrap_or_else(std::sync::PoisonError::into_inner) = Self::next_doc_id();
        b
    }
    #[must_use]
    ...
    fn set_doc_order(&self, value: u64) {
        *self.0.doc_order.write().unwrap_or_else(std::sync::PoisonError::into_inner) = Some(value);
    }

    pub fn set_doc_id_recursive(&self, doc_id: u64) {
        *self.0.doc_id.write().unwrap_or_else(std::sync::PoisonError::into_inner) = doc_id;
        {
            let attrs = self.0.attributes.read().unwrap_or_else(std::sync::PoisonError::into_inner).clone();
            for attr in attrs {
                attr.set_doc_id_recursive(doc_id);
            }
        }
        {
            let namespaces = self.0.namespaces.read().unwrap_or_else(std::sync::PoisonError::into_inner).clone();
            for ns in namespaces {
                ns.set_doc_id_recursive(doc_id);
            }
        }
        {
            let children = self.0.children.read().unwrap_or_else(std::sync::PoisonError::into_inner).clone();
    ...
    fn doc_order(&self) -> Option<u64> {
        *self.0.doc_order.read().unwrap_or_else(std::sync::PoisonError::into_inner)
    }
}
...
impl fmt::Display for SimpleNode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self.kind() {
            ...
            NodeKind::Element => {
                let name = self.name().map_or_else(|| "<unnamed>".to_string(), |q| qname_to_string(&q));
                let attrs = self.attributes().count();
                let ch = self.children().count();
                write!(f, "<{name} attrs={attrs} children={ch}>")
            }
            NodeKind::Attribute => {
                let name = self.name().map_or_else(|| "?".to_string(), |q| qname_to_string(&q));
...
impl SimpleNodeBuilder {
    ...
    pub fn attrs<I: IntoIterator<Item = SimpleNode>>(mut self, attrs: I) -> Self {
        for a in attrs {
            debug_assert_eq!(a.kind(), NodeKind::Attribute);
    ...
    pub fn namespace(mut self, ns: SimpleNode) -> Self {
        debug_assert_eq!(ns.kind(), NodeKind::Namespace);
    ...
    pub fn namespaces<I: IntoIterator<Item = SimpleNode>>(mut self, it: I) -> Self {
        for n in it {
            debug_assert_eq!(n.kind(), NodeKind::Namespace);
    ...
    pub fn value(self, v: &str) -> Self {
        if matches!(
            ...
            *self.node.0.value.write().unwrap_or_else(std::sync::PoisonError::into_inner) = Some(v.to_string());
        }
        self
    }
    #[must_use]
    pub fn build(self) -> SimpleNode {
        ...
        {
            let mut id = self.node.0.doc_id.write().unwrap_or_else(std::sync::PoisonError::into_inner);
            if *id == 0 {
                *id = SimpleNode::next_doc_id();
            }
        }
        {
            let mut nss = self.node.0.namespaces.write().unwrap_or_else(std::sync::PoisonError::into_inner);
            for n in &self.pending_ns {
                *n.0.parent.write().unwrap_or_else(std::sync::PoisonError::into_inner) =
                    Some(Arc::downgrade(&self.node.0));
                let id = *self.node.0.doc_id.read().unwrap_or_else(std::sync::PoisonError::into_inner);
                n.set_doc_id_recursive(id);
            }
            nss.extend(self.pending_ns);
        }
        {
            let mut attrs = self.node.0.attributes.write().unwrap_or_else(std::sync::PoisonError::into_inner);
            for a in self.pending_attrs {
                    ...
                    && let Some(pref) = &qn.prefix
                {
                    ...
                    if let Some(ns_uri) = uri {
                        ...
                        let val = a.0.value.read().unwrap_or_else(std::sync::PoisonError::into_inner).clone();
                        let rebuilt = SimpleNode::new(
                            NodeKind::Attribute,
                            Some(QName { prefix: Some(pref.clone()), local: qn.local.clone(), ns_uri: Some(ns_uri) }),
                            val,
                        );
                        *rebuilt.0.parent.write().unwrap_or_else(std::sync::PoisonError::into_inner) =
                            Some(Arc::downgrade(&self.node.0));
                        let id = *self.node.0.doc_id.read().unwrap_or_else(std::sync::PoisonError::into_inner);
                ...
                if !pushed {
                    *a.0.parent.write().unwrap_or_else(std::sync::PoisonError::into_inner) =
                        Some(Arc::downgrade(&self.node.0));
                    let id = *self.node.0.doc_id.read().unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        {
            let mut ch = self.node.0.children.write().unwrap_or_else(std::sync::PoisonError::into_inner);
            for c in self.pending_children {
                *c.0.parent.write().unwrap_or_else(std::sync::PoisonError::into_inner) =
                    Some(Arc::downgrade(&self.node.0));
                let idc = *self.node.0.doc_id.read().unwrap_or_else(std::sync::PoisonError::into_inner);
...
// Resolves attribute namespace URIs using ancestor bindings once parent links exist.
fn resolve_attr_ns_deep(node: &SimpleNode) {
    if matches!(node.kind(), NodeKind::Element) {
        ...
        {
            let attrs = node.0.attributes.read().unwrap_or_else(std::sync::PoisonError::into_inner);
                    ...
                    if q.ns_uri.is_none() {
                        ...
                        if let Some(ns_uri) = uri {
                            ...
                            *rebuilt.0.parent.write().unwrap_or_else(std::sync::PoisonError::into_inner) =
                            ...
                            let id = *node.0.doc_id.read().unwrap_or_else(std::sync::PoisonError::into_inner);
                            *rebuilt.0.doc_id.write().unwrap_or_else(std::sync::PoisonError::into_inner) = id;
        ...
        if !to_replace.is_empty() {
            let mut attrs_w = node.0.attributes.write().unwrap_or_else(std::sync::PoisonError::into_inner);
...
impl XdmNode for SimpleNode {
    ...
    fn typed_value(&self) -> Vec<XdmAtomicValue> {
        match self.kind() {
            ...
            | NodeKind::Namespace => {
                let value =
                    self.0.value.read().unwrap_or_else(std::sync::PoisonError::into_inner).clone().unwrap_or_default();
                vec![XdmAtomicValue::UntypedAtomic(value)]
            }
            NodeKind::Element | NodeKind::Document => {
                fn dfs(n: &SimpleNode, out: &mut String) {
                    if n.kind() == NodeKind::Text
                        && let Some(v) = &*n.0.value.read().unwrap_or_else(std::sync::PoisonError::into_inner)
                    {
                        out.push_str(v);
                    }
                    for c in n.children() {
                ...
                if let Some(cached) =
                    self.0.cached_text.read().unwrap_or_else(std::sync::PoisonError::into_inner).clone()
                {
                ...
                dfs(self, &mut out);
                *self.0.cached_text.write().unwrap_or_else(std::sync::PoisonError::into_inner) = Some(out.clone());
                vec![XdmAtomicValue::UntypedAtomic(out)]
            }
        }
    }
    fn parent(&self) -> Option<Self> {
        self.0.parent.read().ok()?.as_ref().and_then(std::sync::Weak::upgrade).map(SimpleNode)
    }
    ...
    fn namespaces(&self) -> Self::Namespaces<'_> {
        ...
        if !seen.contains("xml") {
            ...
            *xml.0.parent.write().unwrap_or_else(std::sync::PoisonError::into_inner) =
    ...
    fn compare_document_order(&self, other: &Self) -> Result<core::cmp::Ordering, crate::engine::runtime::Error> {
        let self_doc_id = *self.0.doc_id.read().unwrap_or_else(std::sync::PoisonError::into_inner);
        let other_doc_id = *other.0.doc_id.read().unwrap_or_else(std::sync::PoisonError::into_inner);
