...
unsafe fn enumerate_monitors() -> Result<Vec<MonitorInfo>, PlatformError> {
    extern "system" fn enum_proc(hmonitor: windows::Win32::Graphics::Gdi::HMONITOR, _hdc: HDC, _rc: *mut RECT, lparam: LPARAM) -> BOOL {
        unsafe {
...
unsafe fn monitor_friendly_name(infoex: &MONITORINFOEXW) -> Option<String> {
    ...
    let ok = unsafe {
        EnumDisplayDevicesW(
            windows::core::PCWSTR(infoex.szDevice.as_ptr()),
            0,
            &mut dd,
            0,
        )
...
fn build_friendly_name_map() -> HashMap<String, String> {
    ...
    unsafe {
        ...
        for p in paths {
            ...
            src.header = DISPLAYCONFIG_DEVICE_INFO_HEADER { r#type: DISPLAYCONFIG_DEVICE_INFO_GET_SOURCE_NAME, size: size_of::<DISPLAYCONFIG_SOURCE_DEVICE_NAME>() as u32, adapterId: p.sourceInfo.adapterId, id: p.sourceInfo.id };
            ...
            tgt.header = DISPLAYCONFIG_DEVICE_INFO_HEADER { r#type: DISPLAYCONFIG_DEVICE_INFO_GET_TARGET_NAME, size: size_of::<DISPLAYCONFIG_TARGET_DEVICE_NAME>() as u32, adapterId: p.targetInfo.adapterId, id: p.targetInfo.id };
...
mod tests {
    ...
    fn windows_desktop_info_smoke() {
        ...
        if count > 0 {
            ...
            assert!(info.monitors.iter().all(|m| m.name.as_deref().unwrap_or("").trim().len() >= 1), "monitor names should be present");
        }
    }
}
