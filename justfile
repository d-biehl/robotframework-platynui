...
build-native-wheel *ARGS:
    uv run maturin build --release -m packages/native/Cargo.toml -o dist {{ ARGS }}
...
build-cli-wheel *ARGS:
    uv run maturin build --release -m packages/cli/Cargo.toml -o dist {{ ARGS }}
...
build-inspector-wheel *ARGS:
    uv run maturin build --release -m packages/inspector/Cargo.toml -o dist {{ ARGS }}
