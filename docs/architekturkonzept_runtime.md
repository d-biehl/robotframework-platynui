...
- `DeviceProvider`-Trait + Capability-Typen leben in `crates/core` (Pointer, Keyboard, DesktopInfoProvider, ScreenshotProvider, HighlightProvider); Touch-Unterstützung wird später ergänzt.
  ...
  - `KeyboardDevice` abstrahiert Tastatureingaben. Das Trait liefert mindestens `known_keys() -> &[KnownKeyDescriptor]`, `send_key_event(KeyboardEvent) -> Result<(), KeyboardError>`, `start_input(InputPhase)`, `end_input()`. `start_input`/`end_input` dienen ausschließlich der Tastatur-spezifischen Vorbereitung bzw. dem Aufräumen (z. B. Umschalten des Layouts, Initialisieren eines Text-Composition-Kontextes, Puffern von Wiederholraten) – Fokusverwaltung oder Fensteraktivierung gehört weiterhin in die Runtime/Patterns, nicht in den Provider. Verzögerungen zwischen Events steuert ausschließlich die Runtime (`KeyboardSettings`, `KeyboardOverrides`). `KnownKeyDescriptor` beschreibt Tasten, die der Provider explizit kennt (z. B. `Ctrl`, `Shift`, `Enter`, `F1` …) und kapselt die nativen Codes/Modifier. Namen werden plattformübergreifend abgestimmt: Tasten mit direkter Entsprechung tragen den gleichen Namen auf allen Betriebssystemen (z. B. `Enter`, `Escape`, `Shift`), plattformspezifische Belegungen verwenden etablierte OS-Bezeichnungen (`Command`, `Option`, `Globe` auf macOS; `Windows`-Taste auf Windows; `Meta` oder `Super` auf Linux-Desktopumgebungen). Provider orientieren sich bei der Benennung an den jeweiligen Plattform-Konstanten (Win32 `VK_*`, X11 `XK_*`/`XF86XK_*`, macOS `kVK_*`, etc.) und dokumentieren Abweichungen. Falls eine Plattform eine Taste nicht besitzt, taucht sie in der Liste schlicht nicht auf.
    * Zusätzlich akzeptiert das Trait `KeyboardEvent::RawText` bzw. `KeyboardEvent::RawNamed`, um beliebige Zeichen oder nicht standardisierte Key-Namen zu transportieren (`"ä"`, `"VolumeUp"`, …). Provider entscheiden, wie sie solche Raw-Events umsetzen; klappt dies nicht, signalisiert `KeyboardError::UnsupportedKey/UnsupportedText` die Grenze. Für macOS gilt beispielsweise: `Control` existiert ebenso wie `Command`, `Option`, `Globe`; Windows kennt die `Windows`-Taste, Linux-Desktops nutzen häufig `Super` oder `Meta`. Wichtig ist, dass identische physische Tasten über Betriebssysteme hinweg identische Namen erhalten (z. B. `Control`, `Shift`, `Enter`), damit Sequenzen ausgetauscht werden können. Plattform-spezifische Zusatztasten tragen die offiziellen OS-Namen.
    * Die Runtime bietet darauf aufbauend `KeyboardSequence` als zentrale Repräsentation. Sie parst gemischte Eingaben wie `"eins zwei<Ctrl+a><Ctrl+Delete>Hallo"`, Escapes (`<<`) und Mehrfachshortcuts (`<Ctrl+K Ctrl+C>`) in eine lazy Folge von `KeyEvent`-Strukturen (`press`/`release`). Während des Parsens werden Tastenbezeichner gegen `known_keys()` gematcht – bei Treffern entstehen ` KeyboardEvent::Known`, ansonsten `KeyboardEvent::Raw`.
    * Globale Standardwerte (Press-/Release-Verzögerungen, Nachlauf) werden in `KeyboardSettings` gehalten. Temporäre Abweichungen pro Aufruf laufen über `KeyboardOverrides::builder()`, das ausschließlich Deltas zu den aktiven Settings beschreibt (z. B. anderes Delay, alternative Nachlaufzeit).
...
- `keyboard`: Tastatureingaben über `KeyboardDevice` ausführen; akzeptiert Sequenzen im selben Format wie die Runtime (`"foo<Ctrl+c>bar"`, Escapes via `<<`). Die Runtime erwartet, dass der Client das Ziel zuvor fokussiert hat, und sendet press/release entsprechend der Sequenz.
