...
- **Runtime-Helfer:** `Runtime::evaluate_options(desktop_node)` liefert vorbereitete `EvaluateOptions`, die bereits den Runtime-Resolver enthalten. Aufrufer müssen damit nur noch zusätzliche Optionen setzen (z. B. `with_invalidation(true)`).
