...
- **Desktop-Zusammensetzung:** Plattform-Crates liefern eine `DesktopInfo` (z. B. über `DesktopProvider`), die Auflösung, Monitorliste, Betriebssystemdaten usw. enthält. Die Runtime baut daraus den tatsächlichen `control:Desktop`-Knoten. UiTreeProvider liefern lediglich ihren technologischen Baum (Anwendungen, Fenster, Controls) und geben mit `UiTreeProvider::get_nodes(parent)` jene Knoten zurück, die unterhalb eines vom Runtime-Host bereitgestellten Parents (z. B. Desktop, Anwendung) eingehängt werden sollen. Ein eigener Provider-Desktop existiert nicht.
...
- `crates/core` stellt Traits und Caching-Hilfen (`UiTreeProvider`, `ProviderDescriptor`, `ProviderEvent`, `UiTreeProviderFactory`) bereit. `ProviderDescriptor` beschreibt eine Implementierung (`id`, Anzeigename, `TechnologyId`, `ProviderKind` = `Native` oder `External`, `ProviderPriority` für die spätere Auswahl). `UiTreeProviderFactory::create()` liefert eine `Arc<dyn UiTreeProvider>`-Instanz; zusätzliche Ressourcen werden nicht übergeben. Der Provider verantwortet ausschließlich die Anwendungsebene (`app:`) sowie `control:`/`item:`-Knoten und liefert diese über `UiTreeProvider::get_nodes(parent)` jeweils für den angegebenen Parent zurück; die Runtime kombiniert sie mit der Plattform-Desktop-Node.
- `crates/runtime` ergänzt einen `ProviderRegistry`, der alle via `inventory` registrierten Factories einsammelt, nach Technologie und `ProviderPriority` sortiert und Instanzen erzeugt. Die Registry bietet APIs für die spätere Provider-Auswahl (z. B. „alle Provider einer Technologie“ oder „alle Provider in Prioritätsreihenfolge“).
...
- **NodeResolver-Trait:** Alle Resolver implementieren das Trait `NodeResolver` und geben `Arc<dyn UiNode>` zurück. Eigene Resolver (z. B. JSON-RPC-Proxy) können so Knoten aus alternativen Quellen auflösen, solange sie `RuntimeId`-kompatible Werte liefern.
- **Runtime-Helfer:** `Runtime::evaluate_options(desktop_node)` liefert vorbereitete `EvaluateOptions`, die aktuell lediglich den Desktop-Knoten referenzieren. Einen Resolver müssen Aufrufer – falls benötigt – eigenständig via `with_node_resolver(...)` ergänzen.
...
1. **Core & XPath** – Attributkatalog finalisieren, direkte Wrapper für `UiNode` → `XdmNode` weiter ausbauen und Tests schreiben. Caching/Snapshot-Mechanismen wandern bewusst in den Backlog, bis Performancebedarfe auftreten.
