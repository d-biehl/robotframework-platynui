...
- `pointer`: Zeigeraktionen (Move/Click/Press/Release/Scroll/Drag) über `PointerDevice` ausführen; unterstützt `--origin`, `--motion` sowie Delay-Overrides und bietet `position`, um die aktuelle Desktop-Koordinate des Cursors auszugeben. Zusätzlich lässt sich mit `--move-duration` die gewünschte Dauer für eine Bewegung vorgeben; die Runtime verteilt die Zeit auf die generierten Zwischenpunkte (`PointerProfile::max_move_duration`). Auf Windows setzt die Plattform-Initialisierung die Anwendung vorab auf `DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2`, sodass `SetCursorPos` und `GetCursorInfo` hardwarebezogene Koordinaten liefern.
