...
| `window` | List/control windows (activate, minimize, maximize, restore, close, move, resize) |
| `pointer` | Mouse control (move, click, press, release, scroll, drag, position) |
...
- Options: `--max-depth`, `--attrs default|all|list`, `--include`/`--exclude`, `--exclude-derived`, `--include-runtime-id`, `--pretty`, `--format text|xml`, `--split PREFIX`
