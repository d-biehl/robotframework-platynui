...
- [ ] Falls der Provider eigene Ressourcen hält (Threads, Handles, Verbindungen), implementiert `UiTreeProvider::shutdown()` und gibt diese Ressourcen deterministisch frei. Ein `AtomicBool`‑Guard verhindert doppeltes Shutdown und blockiert Zugriffe nach dem Shutdown (Fehler statt stiller Neuverbindung).
- [ ] Falls das Plattform‑Modul eigene Ressourcen hält (X11‑Connection, Highlight‑Thread, COM‑Singletons), implementiert `PlatformModule::shutdown()` und gibt diese frei. Die Runtime ruft `PlatformModule::shutdown()` nach den Providern auf.
...
- [ ] Fenster, die `WindowSurface` melden, sollen zugleich `Focusable` unterstützen (`IsFocused`-Attribut, Fokus-Aktion). `WindowSurface::activate()/restore()` setzen den Fokus, `minimize()/close()` geben ihn frei (inkl. `NodeUpdated`), damit der Zustand mit nativen Foreground-Wechseln übereinstimmt.
  > Hinweis: Die Erreichbarkeit eines Fensters auf dem aktuellen virtuellen Desktop (Workspace-Wechsel, Fenster-Migration) ist **keine Provider-Verantwortung**. Die Runtime ruft `WindowManager::ensure_window_accessible()` in `bring_to_front()` automatisch auf, bevor `activate()` ausgelöst wird (→ `docs/virtual_desktop_switching.md`).
