...
- [ ] `key_to_code(&str)` löst Namen/Aliasse zuverlässig auf (`KeyboardError::UnsupportedKey` liefert aussagekräftige Hinweise) und `send_key_event(KeyCode, KeyState)` erzeugt korrekte Press/Release-Sequenzen. `start_input`/`end_input` dürfen ausschließlich tastaturspezifische Vor- bzw. Nacharbeiten übernehmen (z. B. Layout-/IME-Umschaltung, Pufferleeren), keine Fokusmanipulation oder Fensteraktivierung.
